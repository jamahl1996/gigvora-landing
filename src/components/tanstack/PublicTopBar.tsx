/**
 * Phase 11 — TanStack-native PublicTopBar.
 * Mirror of src/components/navigation/PublicTopBar.tsx using the RouterLink shim
 * and the TanStack-native MegaMenu.
 */
          ...
          <div className="hidden md:flex items-center gap-1.5">
            {CTA_LINKS.map(cta => (
              <Button key={cta.to} variant="ghost" size="sm" className="text-[10px] h-8 font-medium rounded-xl hover:bg-accent/10 transition-all gap-1" asChild>
                <Link to={cta.to}><cta.icon className="h-3 w-3" />{cta.label}</Link>
              </Button>
            ))}
