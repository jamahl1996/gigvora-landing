/**
 * Phase 11 — TanStack-native AdminTabBar.
 * Mirror of src/components/layout/AdminTabBar.tsx using @tanstack/react-router
 * useLocation/useNavigate and the RouterLink shim.
 */
...
function labelFor(path: string): string {
  ...
  const map: Record<string, string> = {
    cs: 'Customer Service', 'dispute-ops': 'Dispute Ops', finance: 'Finance',
    moderation: 'Moderation', 'trust-safety': 'Trust & Safety',
    'verification-compliance': 'Verification', marketing: 'Marketing',
    'ads-ops': 'Ads Ops', super: 'Super Admin', ops: 'Cross-team Triage',
    search: 'Entity Search', audit: 'Audit Log', flags: 'Feature Flags',
    emergency: 'Emergency', shell: 'Shell', login: 'Login',
  };
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === 'w' && pathname.startsWith('/admin')) {
        ...
        e.preventDefault();
        navigate({ to: '/admin' as unknown as '/' });
      }
    };
  ...
  const closeTab = (path: string) => {
    setTabs((prev) => {
      ...
      if (path === pathname) {
        ...
        navigate({ to: fallback.path as unknown as '/' });
      }
      return next;
    });
  };
      ...
      {orderedTabs.map((tab) => {
        ...
        return (
          <div
            ...
            onClick={() => navigate({ to: tab.path as unknown as '/' })}
            title={tab.path}
          >
