/**
 * Phase 11 — TanStack-native AdminShell.
 * Mirror of src/components/layout/AdminShell.tsx using @tanstack/react-router
 * Outlet/useLocation/useNavigate, the RouterLink shim for Links, and the
 * TanStack-native AdminTabBar / AdminNotificationTray / AdminIsolationGuard
 * / AutoBackNav.
 */
...
import { AdminIsolationGuard } from './AdminIsolationGuard';
...
export const AdminShell: React.FC = () => {
  ...
  const handleLogout = () => {
    ...
    navigate({ to: '/admin/login' as unknown as '/', replace: true });
  };
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active ? 'bg-primary/10 text-primary font-medium' : 'text-foreground/70 hover:text-foreground hover:bg-muted/50',
                )}
              >
                <Icon className="h-4 w-4 shrink-0" />
                {!collapsed && <span className="text-[13px] truncate flex-1">{item.label}</span>}
                {!collapsed && item.badge && (
                  <Badge variant="secondary" className="h-5 min-w-[22px] justify-center px-1.5 text-[10px] font-medium tabular-nums">
                    {item.badge}
                  </Badge>
                )}
                {active && !collapsed && (
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className="bg-background border-b px-4 py-2 flex items-center gap-3">
        <button onClick={() => setCollapsed(!collapsed)} className="p-1.5 rounded-lg hover:bg-muted/60 transition-colors hidden md:block" aria-label="Toggle sidebar">
          {collapsed ? <PanelLeft className="h-4 w-4 text-muted-foreground" /> : <PanelLeftClose className="h-4 w-4 text-muted-foreground" />}
        </button>
        <button onClick={() => setMobileNav(true)} className="p-1.5 rounded-lg hover:bg-muted/60 transition-colors md:hidden" aria-label="Open nav">
        ...
        <button
          onClick={() => navigate({ to: '/admin/search' as unknown as '/' })}
        ...
        >
          ...
          <span className="text-[12px] text-muted-foreground flex-1 text-left">Search cases, users, payouts, campaigns…</span>
        ...
        <div className="relative">
          {isSuper ? (
            <>
              <button onClick={() => setRoleMenuOpen((o) => !o)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border hover:bg-muted/40 transition-colors" aria-haspopup="menu" aria-expanded={roleMenuOpen}>
              ...
              {roleMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-1.5 w-60 rounded-xl border bg-popover shadow-lg z-50 py-1.5">
                    <div className="px-3 py-1.5 text-[10px] uppercase tracking-wider text-muted-foreground border-b mb-1">View as</div>
                    {ROLE_OPTIONS.map((r) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn('w-full flex items-center gap-2.5 px-3 py-2 text-[12px] hover:bg-muted/60 transition-colors', activeRole === r.id && 'bg-primary/10 text-primary font-medium')}
                        >
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border bg-muted/20" title="Role switching is restricted to Super Admins">
              <RoleIcon className="h-3.5 w-3.5 text-foreground" />
              <span className="text-[12px] font-medium hidden md:inline">{roleInfo.label}</span>
        ...
        <div className="flex items-center gap-1.5">
          <button onClick={() => setMessageDialogOpen(true)} className="hidden md:inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border bg-card hover:border-primary/40 hover:bg-primary/5 transition-colors text-[11.5px] font-medium text-foreground/80 hover:text-foreground" title="Message any user on the platform (audit-logged)">
            <Send className="h-3 w-3" />
            Message user
          </button>
          <button onClick={() => setMessageDialogOpen(true)} className="md:hidden p-1.5 rounded-lg hover:bg-muted/60 transition-colors" aria-label="Message any user" title="Message any user">
            <Send className="h-4 w-4 text-muted-foreground" />
          </button>
          <AdminNotificationTray role={activeRole} />
          <a href="/" target="_blank" rel="noopener noreferrer" className="hidden lg:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border bg-background hover:bg-muted/40 transition-colors text-[11px] font-medium text-muted-foreground hover:text-foreground" title="Open the public site in a new tab">
            <ExternalLink className="h-3 w-3" />
            View public site
          </a>
          <button onClick={handleLogout} className="p-1.5 rounded-lg hover:bg-muted/60 transition-colors" aria-label="Sign out" title="Sign out">
            <LogOut className="h-4 w-4 text-muted-foreground" />
          </button>
        </div>
      </header>

      <MessageUserDialog open={messageDialogOpen} onOpenChange={setMessageDialogOpen} asRole={activeRole} />

      <AdminTabBar />

      <div className="flex-1 flex overflow-hidden">
        <aside className={cn('hidden md:flex flex-col border-r bg-muted/15 shrink-0 transition-all duration-200', collapsed ? 'w-12' : 'w-56')}>
